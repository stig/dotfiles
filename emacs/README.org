  ...
  (notmuch-saved-searches
   (quote
    ((:name "Inbox" :query "tag:inbox" :key "i")
     ...
     (:name "Recent" :query "date:2d.." :count-query "date:2d.. and tag:unread" :key "r" :search-type tree)
     (:name "My Threads w/New" :query "thread:\"{from:stig}\" and tag:new" :key "tn" :search-type tree)
     (:name "My Threads" :query "thread:\"{from:stig}\"" :key "ta" :count-query "tag:no-match" :search-type tree))))
