  ...
  (notmuch-saved-searches
   (quote
    ((:name "Inbox" :query "tag:inbox" :key "i")
     ...
     (:name "Org mode Messages" :query "tag:org-mode" :key "o" :sort-order newest-first :search-type tree)
     (:name "Clojure Messages" :query "tag:clojure" :key "c" :sort-order newest-first :search-type tree)
     (:name "TULS Messages" :query "tag:tuls" :key "t" :sort-order newest-first :search-type tree)
     (:name "Recent" :query "date:-4d..today and not tag:lists" :key "r" :count-query "tag:no-match" :sort-order newest-first))))
