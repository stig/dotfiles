...
(use-package ob-plantuml
  ...
  :straight nil
  :custom
  (org-plantuml-jar-path (expand-file-name "/usr/local/opt/plantuml/libexec/plantuml.jar")))
#+end_src
...
#+BEGIN_SRC emacs-lisp
(defun org-agenda-ignore-tag-from-system-name ()
  (if (string-match "margil" (system-name)) "@work" "@home"))
...
(use-package org
  ...
  (org-agenda-custom-commands
   `(("d" "Day Agenda & Top Tasks"
      ...
      ((org-agenda-span 'day)
       (org-agenda-tag-filter-preset '(,(format "-%s" (org-agenda-ignore-tag-from-system-name))
				       "-MAYBE"))))
     ...
     ("gc" "Review for Tasks Complete"
      todo "TODO"
      ((org-agenda-tag-filter-preset '("-MAYBE" "-RECURRING" "-GTD"))
...
(use-package org
  ...
  (org-capture-templates
   '(("t" "TODOs")
     ...
     ("tc" "Plain TODO & Clock in" entry (file "tasks.org") (file "templates/todo.org") :clock-in t :clock-keep t)
     ...
     ("h" "A Haiku" entry (file "") (file "templates/haiku.org") :clock-in t)
...
(use-package org-journal
  ...
  (org-journal-file-header
   (lambda (time)
     (let* ((year-file (format-time-string "%Y.org" time))
	    (year-path (expand-file-name year-file org-journal-dir))
	    (year-title (format-time-string "%Y" time))
	    (year-month-file (format-time-string "%Y-%m.org" time))
	    (year-month-path (expand-file-name year-month-file org-journal-dir))
	    (year-month-title (format-time-string "%B %Y" time)))
       ;; Create the tag files
       (unless (file-exists-p year-path)
	 (with-temp-file year-path
	   (insert "#+title: " year-title)
	   (insert "- tags :: [[file:journal_entries.org][Journal Entries]]")))
       (unless (file-exists-p year-month-path)
	 (with-temp-file year-month-path
	   (insert "#+title: " year-month-title)
	   (insert (format "- tags :: [[file:%s][%s]]" year-file year-title))))
       (format "#+roam_tags: journal
,#+category: journal
- tags :: [[file:%s][%s]]"
	       year-month-file year-month-title))))
...
(use-package notmuch
  ...
  (notmuch-fcc-dirs '(("stig@brautaset.org" . "gandi/sent +sent -inbox -unread")
                      ("stig@circleci.com" . "\"gmail/[Gmail]/Sent Mail\" +sent -inbox -unread")))
  (notmuch-saved-searches
   (quote
    ((:name "Inbox" :query "tag:inbox" :key "i")
     (:name "Flagged" :query "tag:flagged" :key "f")
     (:name "Drafts" :query "tag:draft" :key "d")
     (:name "Recent" :query "date:2d.." :count-query "date:2d.. and not tag:lists" :key "r" :search-type tree)
