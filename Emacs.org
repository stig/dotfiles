...
#+BEGIN_SRC emacs-lisp
(setq machine-tag (if (string-match "margil" (system-name))
		      "gandi" "circleci"))
...
(use-package notmuch
  ...
  (notmuch-saved-searches
   `((:name "Inbox" :query ,(concat "tag:inbox and tag:" machine-tag) :key "i")
     (:name "Flagged" :query ,(concat "tag:flagged and tag:" machine-tag) :key "f")
     (:name "Drafts" :query "tag:draft" :key "d")
     (:name "Recent" :query ,(concat "date:2d.. and not tag:lists and tag:" machine-tag) :key "r" :search-type tree)
