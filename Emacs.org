...
#+end_src
** Fido mode
I used to use Helm, then Ivy, then Selectrum. Now I'm trying the
built-in fido-mode, which is an enhanced icomplete-mode.
...
A templating engine for Emacs. It needs to be initialized before Eglot.
...
It needs Yasnippets to be initialized first, if it's going to use it.
...
Prescient remembers what you selected before, and learns over time, so
it completes better over time.
