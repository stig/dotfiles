...
*** Add Melpa and Org ELPA to ~package-archives~
    ...
    install most packages via Melpa stable, but some from unstable.  I
    need the =org= archive because I use some =org-contrib= stuff that
...
** Clojure
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     Some projects use a `test-' prefix rather than `-test' suffix for
     test files.  This detects those and runs the correct test.  It
     falls back to Cider's default function if the project type is not
     one known to use test- prefixes."
     ...
     (use-package clj-refactor
       :hook ((cider-mode clojure-mode) . clj-refactor-mode)
