...
**** Create a pre-new hook to download messages

     See =notmuch-hooks(5)= for details. I found inspiration for the
     lockfile part of this in answer on [[https://stackoverflow.com/a/31955862/5950][StackOverflow]]. We only get /new/
     mail here, but that's alright because we [[*Run mbsync periodically while Emacs is open][run a full mbsync
     periodically]].

     #+BEGIN_SRC sh :tangle ~/Mail/.notmuch/hooks/pre-new :mkdirp yes :tangle-mode (identity #o755) :prologue "" :epilogue ""
       #!/bin/bash
       pgrep -q -f mbsync || mbsync --quiet --all --pull --new
     #+END_SRC

**** Create post-new hook to tag spam

     It's possible I'm overthinking this, but it's fun to experiment :-)
     ...
     #+BEGIN_SRC sh :tangle ~/Mail/.notmuch/hooks/post-new :mkdirp yes :tangle-mode (identity #o755) :prologue "" :epilogue ""
       ...
       function finish {
	   # If the lockfile was created by us, delete it.
	   [ $$ -eq $(cat $LOCKFILE) ] && rm $LOCKFILE
       }
       trap finish EXIT

       # Write our pid to the lockfile, but fail if file already exists
       (set -o noclobber; echo $$ > $LOCKFILE ) || exit 0

       # Jan 2019; a spammer started sending me spam from dated email
       # addresses. Unfortunately "notmuch query" doesn't let me search for
       # arbitrary headers, otherwise I'd probably use the `X-GND-Spam'
       # one.

       YESTERDAY=$(date -v-1d +%Y%m%d)
       TODAY=$(date +%Y%m%d)
       TOMORROW=$(date -v+1d +%Y%m%d)
       notmuch tag +spam -- from:$YESTERDAY@CONGAPORE.INFO
       notmuch tag +spam -- from:$TODAY@CONGAPORE.INFO
       notmuch tag +spam -- from:$TOMORROW@CONGAPORE.INFO
       notmuch tag +spam -- to:phildonesia7113467883@jpmonfort.int

       # Mute all messages in threads that have at least one muted message
       MUTED_THREADS=$(notmuch search --output=threads tag:muted)
       if [ -n "$MUTED_THREADS" ] ; then
...
*** Composing multiple Email sending profiles
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (setq notmuch-fcc-dirs
	    '(("stig@brautaset.org" . "Gandi/INBOX +sent +archived -unread -inbox")))
...
**** Run mbsync periodically while Emacs is open
     ...
     I use a [[*Create a pre-new hook to download messages][Notmuch hook to fetch new mail]]. However, we also need to
     occasionally push local changes back to the IMAP server. Here we
     set up a timer to periodically run mbsync to do the full sync.
     It's not abnormal for this to take 30 seconds, but usually it's
     less.

     #+BEGIN_SRC emacs-lisp
       (defun sb/mbsync (args)
	 (message "Running mbsync %s..." args)
	 (start-process "mbsync" "*mbsync*" "mbsync" args))

       ;; Capture the timer so we can cancel it later if we need to
       (setq sb/mbsync-timer
	     (run-with-timer 60 900 'sb/mbsync "-qaV"))
     #+END_SRC
