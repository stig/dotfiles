...
** Zsh
   ...
   #+begin_src sh :tangle ~/.zshenv
     ...
     export PATH="$PATH:~/.pyenv/versions/$(cat ~/.pyenv/version)/bin"
...
**** Reading mail with NotMuch
     ...
     notmuch config set user.other_email 'sbrautaset@laterpay.net'
     ...
     notmuch config set search.exclude_tags 'deleted;spam;draft;muted'
     #+end_src
	...
	than just for inboxes."
	  ...
	  (if (> arg 1)
	      (set-process-sentinel
	       (sb/mbsync (if (eq 4 arg) "inboxes" "--all"))
	       (lambda (proc state)
...
***** Tag new messages
	...
	# Mute new messages to already muted threads
