...
I install most packages via Melpa stable, but some from unstable, so I
add them to ~package-archives~. By default it contains only GNU ELPA.
...
MELPA archive by default. For packages that need it I override this by
putting =:pin $repo= in the =use-config= stanza.
