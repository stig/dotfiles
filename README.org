...
** Pre Emacs 27 compat                                               :COMPAT:
   ...
   visible bell. The way I've written my config also require older
   Emacsen to call =package-initialize= at this point.
...
** Add Package Archives & priorities
   ...
   Currently I install most packages via Nix for its roll-back
   ability, but Magit moves too fast for that.
...
**** Sync mail periodically while Emacs is open
     ...
     #+BEGIN_SRC emacs-lisp
       (defun sb/sync-mail ()
	 (interactive)
	 (start-process "*mbsync*" "*mbsync-output*" "mbsync" "--verbose" "--all" "--quiet"))

       (defvar sb/sync-mail-timer nil
         "A variable to let us hold an existing sync timer so we can delete it")

       ;; Cancel any existing sync timer
       (when sb/sync-mail-timer
	 (cancel-timer sb/sync-mail-timer))

       ;; Capture new timer so we can cancel it later above
       (setq sb/sync-mail-timer
	     (run-with-timer 60 10000 'sb/sync-mail))
     #+END_SRC
